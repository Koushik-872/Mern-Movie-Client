...
import { Plus, Edit2, Trash2, Save, Upload, FileJson, AlertCircle, CheckCircle, Download } from 'lucide-react';
...
export const AdminPanel: React.FC = () => {
  ...
  const handleGenreChange = (value: string) => {
    ...
    setFormData({
      ...formData,
      genre: value.split(',').map(g => g.trim()).filter(g => g.length > 0),
    });
  };
  ...
  const handleCastChange = (value: string) => {
    ...
    setFormData({
      ...formData,
      cast: value.split(',').map(c => c.trim()).filter(c => c.length > 0),
    });
  };
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      setImportStatus({
        type: 'success',
        message: response.message || `Successfully imported ${bulkMovies.length} movies!`
      });
      ...
      setImportStatus({
        type: 'error',
        message: error.response?.data?.error || error.message || 'Failed to import movies'
      });
    }
  };
  ...
  const downloadTemplate = () => {
    const template = [
      {
        title: "Example Movie",
        description: "This is a sample movie description that explains the plot.",
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-gray-900 rounded-lg p-6 border border-white/10 space-y-4">
              ...
              <div className="bg-gray-800 rounded p-4 text-sm text-gray-300">
                ...
                <ol className="list-decimal list-inside space-y-1">
                  <li>Download the JSON template or create your own array of movies</li>
                  <li>Each movie must have: title, description, releaseDate, duration, genre (array), director, posterUrl</li>
                  <li>Optional fields: rating (0-10), cast (array), imdbId</li>
              ...
              <div>
                <label className="block text-sm mb-2 font-semibold">Paste JSON Data:</label>
                <textarea
                  value={jsonInput}
              ...
              {importStatus.type && (
                <div className={`flex items-start gap-3 p-4 rounded ${
                  importStatus.type === 'success' ? 'bg-green-600/20 border border-green-600/50' :
                  importStatus.type === 'error' ? 'bg-red-600/20 border border-red-600/50' :
                  'bg-blue-600/20 border border-blue-600/50'
                }`}>
                  {importStatus.type === 'success' && <CheckCircle className="flex-shrink-0 mt-0.5" size={20} />}
                  {importStatus.type === 'error' && <AlertCircle className="flex-shrink-0 mt-0.5" size={20} />}
                  {importStatus.type === 'info' && <AlertCircle className="flex-shrink-0 mt-0.5" size={20} />}
              ...
              {validationErrors.length > 0 && (
                <div className="bg-red-900/20 border border-red-600/50 rounded p-4">
                  <p className="font-semibold mb-2 text-red-400">Validation Errors:</p>
              ...
              {bulkMovies.length > 0 && validationErrors.length === 0 && (
                <div className="bg-gray-800 rounded p-4">
                  <p className="font-semibold mb-3">Preview ({bulkMovies.length} movies):</p>
                  <div className="max-h-60 overflow-y-auto space-y-2">
                    {bulkMovies.map((movie, index) => (
                      <div key={index} className="bg-gray-700/50 rounded p-3 text-sm">
                        <p className="font-semibold">{index + 1}. {movie.title}</p>
                        <p className="text-gray-400 text-xs mt-1">
                          {movie.director} • {movie.genre?.join(', ')} • {movie.duration}min
                        </p>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={bulkMovies.length === 0 || validationErrors.length > 0}
          ...
          {showForm && (
            <div className="bg-gray-900 rounded-lg p-6 border border-white/10">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm mb-2">Genres (comma-separated) *</label>
                  <input
                    type="text"
                    required
                    value={genreInput}
                ...
                <div>
                  <label className="block text-sm mb-2">Cast (comma-separated)</label>
                  <input
                    type="text"
                    value={castInput}
                ...
                <div>
                  <label className="block text-sm mb-2">IMDB ID (optional)</label>
                  <input
                    type="text"
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              <h2 className="text-2xl font-bold">All Movies ({movies.length})</h2>
              <div className="bg-gray-900 rounded-lg border border-white/10 overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody>
                    {movies.map((movie) => (
                      <tr key={movie._id} className="border-t border-white/10 hover:bg-white/5 transition-colors">
                        <td className="p-4">
                          <img
                            src={movie.posterUrl}
                            alt={movie.title}
                            ...
                            onError={(e) => {
                              (e.target as HTMLImageElement).src = 'https://via.placeholder.com/100x150/1a1a1a/ffffff?text=' + movie.title;
                            }}
                          />
                        </td>
                        <td className="p-4 font-medium">{movie.title}</td>
                        <td className="p-4 text-white/80">{movie.director}</td>
                        <td className="p-4 text-white/80">{movie.genre.join(', ')}</td>
                        <td className="p-4">
